...
class CameraAppPixmix(CameraAppBase):
    """Dual camera pixel mixing app"""
    ...
    def start(self):
        ...
        print("Starting Pixmix camera app...")
        ...
        self.preview_config0 = self.cam0.create_preview_configuration(
            main={"size": (self.board.LCD_WIDTH, self.board.LCD_HEIGHT)}
        )
        ...
        self.still_config0 = self.cam0.create_still_configuration(
            main={"size": self.cam0.sensor_resolution}
        )
        self.still_config1 = self.cam1.create_still_configuration(
            main={"size": self.cam1.sensor_resolution}
        )
        ...
        # Enable auto controls for both cameras
        self.cam0.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})  # AfMode 2 = Continuous
        self.cam1.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})
    ...
    def stop(self):
        ...
        print("Stopping Pixmix camera app...")
        if self.cam0:
            self.cam0.stop()
            self.cam0.close()
            self.cam0 = None
    ...
    def capture_images(self):
        ...
        # Create subfolder for pixmix captures in the script's directory
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        output_dir = os.path.join(script_dir, "pixmix_captures")
        ...
        # Re-enable auto controls after reconfiguration
        self.cam0.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})
        self.cam1.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})
        ...
        # Get image data from main stream (full resolution)
        arr0 = req0.make_array("main")
        arr1 = req1.make_array("main")
        
        # Rotate cam1 by 180 degrees (physically rotated camera)
        img1_temp = Image.fromarray(arr1)
        img1_temp = img1_temp.rotate(180)
        arr1 = np.array(img1_temp)
        ...
        # Rotate final image 90 degrees clockwise
        img_mixed = img_mixed.rotate(-90, expand=True)
        
        # Save mixed JPG
        filename = f"{output_dir}/{timestamp}_pixmix_v.jpg"
        ...
        # Re-enable auto controls after switching back to preview
        self.cam0.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})
        self.cam1.set_controls({"AfMode": 2, "AwbEnable": True, "AeEnable": True})
    ...
    def update(self):
        """Update display with camera 2 preview"""
        if self.exit_requested:
            return False  # Signal to exit
        
        try:
            # Grab frame from camera 1 (camera 2) for preview
            ...
            if img1.size != (self.board.LCD_WIDTH, self.board.LCD_HEIGHT):
                img1 = img1.resize((self.board.LCD_WIDTH, self.board.LCD_HEIGHT))
            
            # Convert to RGB565 byte list
            rgb565_data = self.pil_to_rgb565(img1)
            ...
            if self.shared_state is not None:
                ...
                buf = BytesIO()
                img1.save(buf, format="JPEG", quality=70)
